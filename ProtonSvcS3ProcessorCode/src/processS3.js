...
const { getProtonTemplateName } = require('./protonsvc')
...
const processS3 = async (record) => {
  try {
    const protonTemplateName = await getProtonTemplateName(process.env.PROTON_SVC_TEMPLATE)
    console.log(protonTemplateName);

    // if there is no template name OR it's different
    if (!protonTemplateName || protonTemplateName != process.env.PROTON_SVC_TEMPLATE) {
      // create a new template
      let protonResponse = await proton.createServiceTemplate({
    ...
    var params = {
      ...
      {
      majorVersion: majorVersion, /* required */
      templateName: process.env.PROTON_SVC_TEMPLATE /* required */
    },
    ]
    };
